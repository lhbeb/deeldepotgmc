      ...
      <div className="flex w-24 gap-0.5">
        {Array.from({ length: 5 }).map((_, i) => (
          <StarShape
            key={i}
            size={12}
            fill={i < filledStars ? "#f5970c" : "transparent"}
            className={i < filledStars ? "text-[#f5970c]" : "text-zinc-200"}
          />
        ))}
      </div>
      <div className="h-2 flex-1 overflow-hidden rounded-full bg-zinc-100">
        <div
          className="h-full rounded-full bg-[#f5970c] transition-all duration-1000"
          style={{ width: `${percent}%` }}
        />
      </div>
...
function ReviewStarsBlue() {
  return (
    <div className="mb-2 flex gap-0.5">
      {Array.from({ length: 5 }).map((_, i) => (
        <StarShape key={i} size={14} fill="#f5970c" className="text-[#f5970c]" />
      ))}
    </div>
  );
}
          ...
          item === "ellipsis" ? (
            ...
            <button
              ...
              className={`h-10 w-10 rounded-md text-sm font-bold transition-all ${
                page === item ? "bg-[#030B19] text-[#F0F6FF]" : "text-zinc-600 hover:bg-zinc-50"
              }`}
            >
              {item}
            </button>
    ...
    >
      <div className="mx-auto max-w-4xl">
        ...
        <div className="mb-6 flex flex-col items-center">
          <div className="mb-2 text-5xl font-black text-[#030B19]">{customerReviewSummary.average}</div>
          <div className="mb-1 flex gap-1">
            {Array.from({ length: 5 }).map((_, i) => (
              <StarShape key={i} size={20} fill="#f5970c" className="text-[#f5970c]" />
            ))}
          </div>
            ...
            >
              <div className="mb-4 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="flex h-10 w-10 items-center justify-center rounded-full bg-[#F0F6FF] font-black text-[#030B19]">
                    {r.initial}
                  </div>
                  <div>
