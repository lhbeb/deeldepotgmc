...
export default function AdminProductsPage() {
  ...
  const handleExportAllCSV = async () => {
    ...
    try {
      ...
      a.download = `Deel Depot-all-products-${date}.csv`;
      document.body.appendChild(a);
      a.click();
      a.remove();
      window.URL.revokeObjectURL(url);
  ...
  const handleExportGoogleShoppingCSV = async () => {
    ...
    try {
      ...
      const rows = gmcProducts.map((p: any) => {
        ...
        const brand = p.brand || 'Deel Depot';
  ...
  const handleExportAffiliateJSON = async () => {
    ...
    try {
      ...
      a.href = url;
      const date = new Date().toISOString().slice(0, 10);
      a.download = `Deel Depot-affiliate-json-${date}.json`;
      document.body.appendChild(a);
      a.click();
      a.remove();
      window.URL.revokeObjectURL(url);
                  ...
                  <td className="px-4 py-3 text-center hidden lg:table-cell">
                    <button
                      ...
                      className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium transition-colors ${(product.isFeatured || product.is_featured)
                        ? 'bg-[#030B19]/10 text-[#030B19] hover:bg-[#030B19]/15'
                        : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
                        } disabled:opacity-50`}
                      title={(product.isFeatured || product.is_featured) ? 'Remove from featured' : 'Add to featured'}
                    >
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center justify-end gap-1 relative">
                      ...
                      <div className="hidden xl:flex items-center gap-1">
                        ...
                        >
                          {togglingFeatured === product.slug ? (
                            <RefreshCw className="h-4 w-4 text-[#030B19] animate-spin" />
                          ) : (
                            <Star className={`h-4 w-4 ${(product.isFeatured || product.is_featured) ? 'text-[#030B19] fill-[#030B19]' : 'text-gray-500'}`} />
                          )}
                        </button>
                        <Link
                          href={`/admin/products/${product.slug}/edit`}
