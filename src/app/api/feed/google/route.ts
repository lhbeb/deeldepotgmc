  ...
  try {
    ...
    const itemsXml = products
      ...
      .map((product) => {
        ...
        const brand = escapeXml(product.brand || 'Deel Depot');
...
<rss version="2.0" xmlns:g="http://base.google.com/ns/1.0">
  <channel>
    <title>Deel Depot Google Merchant Center Feed${targetLabel}${currencyLabel}</title>
    <link>${BASE_URL}</link>
    <description>Selected Deel Depot products for ${country || 'United Kingdom and United States'}${currencyLabel}</description>
    ${itemsXml}
  </channel>
</rss>`;
