...
function getSafeStripeError(error: any): string {
    ...
    if (isSensitive) {
        ...
        return 'Payment processing is temporarily unavailable. Please email contact@deeldepot.com';
    }

    // For non-sensitive errors, we can show a slightly more specific message
    // but still avoid technical jargon
    if (error.type === 'card_error') {
        return 'There was an issue with your payment method. Please try a different card or email contact@deeldepot.com';
    }

    // Generic fallback for any other errors
    return 'An error occurred during payment processing. Please email contact@deeldepot.com';
}
