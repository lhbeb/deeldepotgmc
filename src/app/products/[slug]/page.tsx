...
export async function generateMetadata(
  ...
  try {
    ...
    if (!slug) return { title: 'Product Not Found | Deel Depot' };

    let product = isReviewProduct(slug) ? getReviewProduct(slug) : null;
    if (!product) product = await getProductBySlug(slug);
    if (!product) return { title: 'Product Not Found | Deel Depot' };

    const title = `${product.title || 'Product'} - ${product.brand || ''} | ${product.category || ''} | Deel Depot`;
    ...
    return {
      title: 'Product | Deel Depot',
      description: 'Browse our products on Deel Depot',
    };
  }
}
