    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center px-4">
          ...
          <Link
            href="/"
            className="inline-block bg-[#030B19] hover:bg-[#0a0f32] text-[#F0F6FF] px-6 py-3 rounded-lg transition-colors duration-300"
          >
            Return to Home
          </Link>
        </div>
        ...
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:items-start">
            <div className="relative lg:sticky lg:top-0 lg:self-start">
                  ...
                  {images.map((image, idx) => (
                    image ? (
                      <button
                        ...
                        className={`relative flex-shrink-0 w-20 h-20 rounded-md overflow-hidden ${activeImage === idx ? 'ring-2 ring-[#030B19]' : 'ring-1 ring-gray-200'}`}
                      >
                        <Image
                          src={image}
              ...
              {images.length > 1 && (
                <>
                  <button onClick={() => setActiveImage((prev) => (prev > 0 ? prev - 1 : images.length - 1))} className="absolute left-4 top-1/2 -translate-y-1/2 transform bg-white/80 hover:bg-[#030B19] hover:text-[#F0F6FF] p-2 rounded-full transition-all duration-300 z-10">
                    <ChevronLeft className="h-6 w-6" />
                  </button>
                  <button onClick={() => setActiveImage((prev) => (prev < images.length - 1 ? prev + 1 : 0))} className="absolute right-4 top-1/2 -translate-y-1/2 transform bg-white/80 hover:bg-[#030B19] hover:text-[#F0F6FF] p-2 rounded-full transition-all duration-300 z-10">
            ...
            <div className="lg:pr-4">
                ...
                <div className="mt-3 w-fit max-w-full">
                  ...
                  >
                    ...
                    {getConditionTooltip(condition) && isConditionTooltipVisible && (
                      <div
                        className="pointer-events-none z-[70] w-72 max-w-[calc(100vw-2rem)] rounded-xl border border-[#030B19]/10 bg-[#030B19] px-3 py-2 text-xs leading-5 text-[#F0F6FF] shadow-xl"
                        style={conditionTooltipStyle}
                      >
                        {getConditionTooltip(condition)}
                        <div className="absolute bottom-full left-5 border-4 border-transparent border-b-[#030B19] md:bottom-auto md:left-[-8px] md:right-auto md:top-1/2 md:-translate-y-1/2 md:border-b-transparent md:border-r-[#030B19] md:border-l-transparent"></div>
                      </div>
                    )}
                  </div>
                </div>
              ...
              <ClientOnly>
                {viewedCount !== null && viewedCount > 0 && (
                  <div className="mt-6 bg-[#030B19]/10 border border-[#030B19]/30 rounded-xl p-3 sm:p-4">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-2 sm:space-x-4">
                        <div className="flex items-center text-[#030B19]">
                      ...
                      <div className="flex items-center">
                        <div className="w-2 h-2 bg-[#030B19] rounded-full animate-pulse mr-2"></div>
                        <span className="text-xs text-[#030B19] font-medium hidden sm:inline">Live activity</span>
                      </div>
                    </div>
                  </div>
                )}
                  ...
                  <>
                    ...
                    {(product.checkoutFlow === 'paypal-invoice' || product.checkoutFlow === 'paypal-unclaimed' || product.checkoutFlow === 'paypal-direct' || product.checkoutFlow === 'paypal-api') ? (
                      ...
                      <button
                        onClick={handleBuyNow}
                        disabled={isAddingToCart || isBuyingNow}
                        className="hidden lg:flex w-full bg-transparent border-2 border-[#030B19] hover:border-[#0a0f32] text-[#030B19] hover:text-[#0a0f32] py-4 px-6 rounded-xl font-semibold transition-colors duration-200 items-center justify-center disabled:opacity-50 disabled:cursor-not-allowed"
                      >
                        {isBuyingNow ? (
                          <>
                            <div className="animate-spin rounded-full h-5 w-5 border-b-2 border-[#030B19] mr-2"></div>
                            Processing...
                          </>
                        ) : (
                          <>
                ...
                <div className="rounded-[20px] border border-[#E8EEF8] bg-white px-5 py-5">
                  ...
                  {shouldCollapseDescription && (
                    <button
                      ...
                      className="mt-4 text-sm font-semibold text-[#030B19] transition hover:text-[#00297F]"
                    >
            ...
            <section className="rounded-[24px] border border-[#DCE5F5] bg-white px-8 py-8">
              ...
              {shouldCollapseDescription && (
                <button
                  type="button"
                  onClick={() => setShowFullDescription((current) => !current)}
                  className="mt-5 text-sm font-semibold text-[#030B19] transition hover:text-[#00297F]"
                >
                  {showFullDescription ? "Show less" : "Show more"}
                </button>
              )}
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        ...
                        <span className="mt-0.5 flex-shrink-0 text-[#030B19]" aria-hidden="true">
                      ...
                      {isOpen && (
                        <div className="pt-3 text-sm leading-7 text-[#5B6785]">
                          ...
                          {item.linkHref && item.linkLabel && (
                            <Link
                              href={item.linkHref}
                              className="mt-2 inline-flex text-sm font-semibold text-[#030B19] transition hover:text-[#00297F]"
                            >
                              {item.linkLabel}
                            </Link>
                          )}
              ...
              {faqItems.length > 4 && (
                <div className="border-t border-[#E8EEF8] px-6 py-5 sm:px-8">
                  <button
                    ...
                    className="text-sm font-semibold text-[#030B19] transition hover:text-[#00297F]"
                  >
        ...
        <div className="fixed inset-0 bg-black bg-opacity-90 z-50" onClick={() => setShowZoom(false)}>
          ...
          <div className="absolute inset-0 flex items-center justify-center p-4" onClick={(e) => e.stopPropagation()} onTouchStart={handleTouchStart} onTouchMove={handleTouchMove} onTouchEnd={handleTouchEnd}>
            <div className="relative w-full h-full">
              ...
              {images.length > 1 && (
                <>
                  <button onClick={(e) => { e.stopPropagation(); setActiveImage((prev) => (prev > 0 ? prev - 1 : images.length - 1)); setZoomLevel(1); }} className="absolute left-4 top-1/2 -translate-y-1/2 transform bg-white/10 hover:bg-[#030B19] p-3 rounded-full text-white transition-colors duration-200" aria-label="Previous image"><ChevronLeft className="h-8 w-8" /></button>
                  <button onClick={(e) => { e.stopPropagation(); setActiveImage((prev) => (prev < images.length - 1 ? prev + 1 : 0)); setZoomLevel(1); }} className="absolute right-4 top-1/2 -translate-y-1/2 transform bg-white/10 hover:bg-[#030B19] p-3 rounded-full text-white transition-colors duration-200" aria-label="Next image"><ChevronRight className="h-8 w-8" /></button>
                </>
              )}
            </div>
          </div>
