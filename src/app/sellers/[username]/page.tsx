...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!seller) {
    return {
      title: 'Seller Not Found | Deel Depot',
      description: 'The requested seller profile could not be found.',
    };
  }

  return {
    title: `${seller.name} | Deel Depot Seller`,
    description: seller.bio || `Shop products from ${seller.name} on Deel Depot.`,
    openGraph: {
      title: `${seller.name} - Deel Depot`,
      description: seller.bio || `Check out ${seller.name}'s profile and listings on Deel Depot.`,
